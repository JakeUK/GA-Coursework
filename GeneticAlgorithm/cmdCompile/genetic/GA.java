	...
	public void iterateGeneration() {
		...
		for(int i = topRanks; i < settings.POPULATION_SIZE; i++) {
			...
			if(parents[0] != null && parents[1] != null) {
				...
				System.out.println("No parents found | Check fitness function");
		...
		for(int i = 0; i < newCandidates.size(); i++) {
			if(i != 0) {
				newCandidates.get(i).crossover(); // First place does not get mutated
			}
			newCandidates.get(i).calculateFitness(true);
		}
